from config import BASE_DIRECTORY, TOKEN, CHAT_ID
...
def portfolio_changes_etl_excel(file_name):
...
def process_portfolio_changes(file_name, portfolio_changes, engine, connection, sql_active_stocks):   
    ...
    processed_dir = BASE_DIRECTORY + 'processed_portfolio_changes'
...
async def telegram_send_updates(engine):
    bot = Bot(token=TOKEN)
    ...
    # ### Send latest visual reports:
    # image_paths = [visual_reports_dir+'\\combined_profits.png', visual_reports_dir+'\\growth.png', visual_reports_dir+'\\total_portfolio.png']

    # # List to hold the media group
    # media_group = []

    # # Generate media group data
    # for i, image_path in enumerate(image_paths):
    #     media_group.append(('media', (f'photo{i+1}.png', open(image_path, 'rb'), 'image/png')))

    # # Create a 'multipart/form-data' encoder
    # multipart_data = MultipartEncoder(
    #     fields={
    #         'chat_id': CHAT_ID,
    #         'media': str([
    #             {'type': 'photo', 'media': f'attach://photo{i+1}.png'} for i in range(len(image_paths))
    #         ]),
    #         **{f'photo{i+1}.png': ('filename', open(path, 'rb'), 'image/png') for i, path in enumerate(image_paths)}
    #     }
    # )

    # # Proper headers must be set for 'multipart/form-data'
    ...
    # ax.axis('tight')
    # ax.axis('off')
    # ax.table(cellText=profit_over_time_df.values, colLabels=profit_over_time_df.columns, loc='center')
    # plt.savefig('temp_df_image.png')
    # plt.close()
